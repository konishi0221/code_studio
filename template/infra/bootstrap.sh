...
# apps/, infra/, firebase.json).
#
# template/ がまだ親 repo の subdir として配置されている過渡期は BASE_DIR=template
# でこの repo を直接動かせる。後で template/ を別 repo に extract したら BASE_DIR=.
# (= リポジトリ root) を指定して走らせる。
BASE_DIR="${BASE_DIR:-template}"
